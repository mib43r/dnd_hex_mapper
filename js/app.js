$(function () {
  ...
  window.updateStatus = function updateStatus() { updateInfluenceSummary(); };
  ...
  $('#calculateInfluences').on('click', function () {
    ...
    renderMap();
    const total = INFLUENCE_TYPES.reduce((sum, type) => sum + Object.keys(state.influences.costs[type]).length, 0);
    setStatus(`Calculated forest, grain, and city influence for ${total} type-cell reaches.`);
  });

  $('#clearInfluences').on('click', function () { invalidateInfluences(); renderMap(); setStatus('Influence overlays cleared.'); });
  ...
  $('.influence-setting').on('change', function () {
    if (!updateInfluenceSettingsFromControls()) return setStatus('Influence budgets and terrain multipliers must be greater than zero.');
  ...
  $('#zoomOut').on('click', () => { interaction.zoom = Math.max(0.5, interaction.zoom / 1.25); applyViewBox(); });
  $('#fitMap').on('click', () => { interaction.zoom = 1; interaction.panX = 0; interaction.panY = 0; applyViewBox(); });

  $('#hexMap').on('wheel', function (event) {
    event.preventDefault();
    interaction.zoom = Math.max(0.5, Math.min(6, interaction.zoom * (event.originalEvent.deltaY < 0 ? 1.12 : 0.89)));
    applyViewBox();
  });
  ...
  $('#hexMap').on('pointermove', function (event) {
    if (interaction.panning && interaction.baseViewBox) {
      ...
      interaction.panX = interaction.panStart.panX - (event.clientX - interaction.panStart.x) * ((interaction.baseViewBox.width / interaction.zoom) / rect.width);
      interaction.panY = interaction.panStart.panY - (event.clientY - interaction.panStart.y) * ((interaction.baseViewBox.height / interaction.zoom) / rect.height);
      applyViewBox(); return;
    }
    updateCellGesture(event);
  });
  $('#hexMap').on('pointerup', function (event) {
    interaction.panning = false; finishCellGesture(event);
    if (this.hasPointerCapture?.(event.pointerId)) this.releasePointerCapture(event.pointerId);
  });
  $('#hexMap').on('pointercancel', function (event) {
    interaction.panning = false; finishCellGesture(event, true);
    if (this.hasPointerCapture?.(event.pointerId)) this.releasePointerCapture(event.pointerId);
  });
